...
import { 
  FiGlobe, FiCloud, FiGrid, FiTerminal, 
  FiArrowRight, FiGithub, FiTwitter, FiLinkedin, FiMail,
  FiCheckCircle, FiCode, FiDatabase, FiCpu,
  FiStar, FiZap
} from 'react-icons/fi';
    ...
    <div className="w-full h-[500px] rounded-xl overflow-hidden border border-gray-800 bg-black/50 relative">
      <Canvas camera={{ position: [0, 0, 3.5], fov: 45 }}>
        ...
        <Sphere args={[1.22, 64, 64]}>
          <meshStandardMaterial color="#3b82f6" transparent opacity={0.1} side={THREE.BackSide} />
        </Sphere>
          ...
          return (
            <group key={country.name}>
              <mesh position={pos}>
                ...
                <meshStandardMaterial color={country.color} emissive={country.color} emissiveIntensity={0.8} transparent opacity={0.8} />
              </mesh>
              <mesh position={pos}>
        ...
        <OrbitControls enableZoom enablePan={false} autoRotate autoRotateSpeed={1.5} enableDamping dampingFactor={0.05} />
      </Canvas>
...
const AutoLaunchRocket = () => {
  ...
  useEffect(() => {
    const initialTimer = setTimeout(() => setLaunchState('launching'), 1500);
  ...
  return (
    <div key={resetKey} className="relative flex justify-center items-center h-64">
      ...
      {launchState === 'launching' && (
        <div className="absolute inset-0 pointer-events-none">
          {Array.from({ length: 20 }).map((_, i) => (
            <div key={i} className="absolute w-1 h-1 bg-orange-400 rounded-full"
              style={{
      ...
      <style>{`@keyframes fadeUp { 0% { opacity: 1; transform: translateY(0); } 100% { opacity: 0; transform: translateY(-80px); } }`}</style>
    </div>
  );
};

// ---------- ANIMATED CODE (fixed with explicit type for visibleLines) ----------
...
const Partners = () => {
  ...
  return (
    <section className="py-16 px-6 border-y border-gray-800 bg-gray-900/20">
      <div className="max-w-7xl mx-auto"><div className="text-center mb-12"><h2 className="text-3xl font-bold mb-3">Trusted by leading ecosystems</h2><p className="text-gray-400">We partner with organizations that empower African tech talent</p></div>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
        {partners.map((partner, idx) => (
          <div key={idx} className={`bg-gradient-to-br ${partner.bg} border border-gray-800 rounded-xl p-4 text-center backdrop-blur-sm hover:scale-105 transition`}>
            <div className="text-3xl mb-2">{partner.logo}</div><div className="text-sm font-medium">{partner.name}</div>
          </div>
        ))}
      </div></div>
    </section>
...
const Testimonials = () => {
  const testimonials = [
    { name: "Amara Okonkwo", role: "CTO, Swiftly Logistics", content: "PCLOUDs reduced our deployment time by 70%. The API marketplace gave us access to SMS and payment APIs in minutes, not weeks.", avatar: "👩‍💻", rating: 5 },
    { name: "Kwame Asante", role: "Full-stack developer, Freelance", content: "As a solo developer, I love the startup bundle. Domain + hosting + API credits for $19/mo is unbeatable. The CLI is buttery smooth.", avatar: "👨‍💻", rating: 5 },
    { name: "Fatima El-Sayed", role: "Founder, EduTech Kenya", content: "We moved all our student projects to PCLOUDs. The static hosting and database tools just work. Support is responsive and helpful.", avatar: "👩‍🏫", rating: 5 },
    { name: "Tunde Adebayo", role: "DevOps Engineer, Paystack", content: "Finally, a cloud platform built with African developers in mind. The API documentation is top-notch and the local pricing makes sense.", avatar: "🧑‍🔧", rating: 5 }
  ];
  return (
    <section className="py-20 px-6 bg-gray-900/40">
      <div className="max-w-7xl mx-auto"><div className="text-center mb-12"><h2 className="text-3xl font-bold mb-3">Loved by developers across Africa</h2><p className="text-gray-400">Join thousands of engineers building the future on PCLOUDs</p></div>
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        {testimonials.map((t, idx) => (
          <div key={idx} className="bg-gray-800/30 border border-gray-700 rounded-xl p-5 hover:border-gray-600 transition">
            <div className="flex items-center gap-3 mb-3"><div className="text-3xl">{t.avatar}</div><div><div className="font-semibold">{t.name}</div><div className="text-xs text-gray-400">{t.role}</div></div></div>
            <div className="flex mb-2">{[...Array(t.rating)].map((_, i) => <FiStar key={i} className="text-yellow-400 fill-yellow-400" />)}</div>
  ...
  return (
    <section className="py-12 px-6 border-y border-gray-800 bg-black/20">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-6 text-center">
        <div><div className="text-3xl md:text-4xl font-bold text-blue-400">{counts.developers.toLocaleString()}+</div><div className="text-gray-500 text-sm mt-1">Developers</div></div>
        <div><div className="text-3xl md:text-4xl font-bold text-purple-400">{counts.apis}+</div><div className="text-gray-500 text-sm mt-1">APIs available</div></div>
        <div><div className="text-3xl md:text-4xl font-bold text-green-400">{counts.deployments.toLocaleString()}+</div><div className="text-gray-500 text-sm mt-1">Monthly deployments</div></div>
        <div><div className="text-3xl md:text-4xl font-bold text-red-400">{counts.uptime}%</div><div className="text-gray-500 text-sm mt-1">Uptime SLA</div></div>
      </div>
    </section>
  );
};
...
const GlobalReach = () => {
  return (
    <section className="py-20 px-6 bg-gray-900/20">
      <div className="max-w-7xl mx-auto"><div className="text-center mb-10"><h2 className="text-3xl font-bold mb-3">Our footprint across Africa</h2><p className="text-gray-400">Serving developers in 12 countries and growing</p></div>
      <div className="grid lg:grid-cols-2 gap-10 items-center">
        <Globe3D />
        <div className="space-y-4">
          <div className="bg-gray-800/30 rounded-xl p-5 border border-gray-700"><h3 className="text-xl font-semibold mb-3 flex items-center gap-2"><FiGlobe className="text-blue-400" /> Active Regions</h3><div className="grid grid-cols-2 gap-2">{countryMarkers.map(c => (<div key={c.name} className="flex items-center gap-2 text-sm"><div className="w-2 h-2 rounded-full" style={{ backgroundColor: c.color }}></div><span>{c.name}</span></div>))}</div></div>
          <div className="bg-gradient-to-r from-blue-600/10 to-purple-600/10 rounded-xl p-5 border border-blue-500/30"><p className="text-gray-300 text-sm">🌍 We're expanding across the continent. Next: Rwanda, Zambia, Botswana, and Senegal.</p></div>
        </div>
      </div></div>
    </section>
  );
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-gray-950/90 backdrop-blur-md border-b border-gray-800' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2"><div className="w-8 h-8 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center"><FiCloud className="text-white" /></div><span className="font-bold text-xl tracking-tight">PCLOUDs</span><span className="text-xs bg-blue-600/20 text-blue-400 px-2 py-0.5 rounded-full ml-2">beta</span></div>
        <div className="hidden md:flex items-center gap-6 text-gray-300"><a href="#domains" className="hover:text-white transition">Domains</a><a href="#hosting" className="hover:text-white transition">Hosting</a><a href="#api-marketplace" className="hover:text-white transition">APIs</a><a href="#library" className="hover:text-white transition">Library</a><a href="#cli" className="hover:text-white transition">CLI</a><a href="#partners" className="hover:text-white transition">Partners</a><a href="#docs" className="hover:text-white transition">Docs</a></div>
        <div className="flex items-center gap-3"><button className="text-gray-300 hover:text-white px-3 py-1.5 rounded-md transition">Sign in</button><button className="bg-blue-600 hover:bg-blue-500 px-4 py-1.5 rounded-md font-medium transition shadow-lg shadow-blue-600/20">Get Started</button></div>
      </div>
    </nav>
  );
};
...
const Hero = () => {
  ...
  return (
    <section className="relative pt-32 pb-20 px-6 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10"><div className="grid md:grid-cols-2 gap-12 items-center">
        <div><div className="inline-flex items-center gap-2 bg-gray-900/50 border border-gray-800 rounded-full px-3 py-1 text-sm mb-6"><span className="relative flex h-2 w-2"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span></span><span>Africa's first developer cloud</span></div>
        ...
        <div className="flex flex-wrap gap-4"><button className="bg-blue-600 hover:bg-blue-500 px-6 py-3 rounded-lg font-medium flex items-center gap-2 transition">Start building <FiArrowRight /></button><button className="border border-gray-700 hover:border-gray-500 px-6 py-3 rounded-lg font-medium flex items-center gap-2 transition"><FiTerminal /> pcloud deploy</button></div>
        <div className="mt-8 flex items-center gap-6 text-sm text-gray-500"><span className="flex items-center gap-1"><FiCheckCircle className="text-green-500" /> No credit card required</span><span className="flex items-center gap-1"><FiZap className="text-yellow-500" /> 99.9% uptime SLA</span></div></div>
        <div className="relative"><div className="gradient-border"><div className="gradient-border-inner"><div className="flex items-center gap-2 border-b border-gray-800 pb-3 mb-4"><div className="flex gap-1.5"><div className="w-3 h-3 rounded-full bg-red-500"></div><div className="w-3 h-3 rounded-full bg-yellow-500"></div><div className="w-3 h-3 rounded-full bg-green-500"></div></div><span className="text-xs text-gray-500">bash</span></div><AnimatedCode lines={codeLines} /></div></div><div className="absolute -top-8 -right-8 text-7xl text-blue-500/10 font-mono select-none animate-pulse-slow">{'</>'}</div></div>
      </div></div>
    </section>
  );
};
...
const FeatureCard = ({ title, description, gradient }: FeatureCardProps) => (
  <div className="group relative bg-gray-900/50 border border-gray-800 rounded-xl p-6 hover:border-gray-700 transition-all duration-300 hover:-translate-y-1">
    <div className={`w-12 h-12 bg-gradient-to-br ${gradient} rounded-lg flex items-center justify-center mb-4 group-hover:scale-110 transition`}></div>
...
const ModulesSection = () => {
  ...
  return (
    <section className="py-20 px-6" id="features">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">{modules.map((mod, idx) => (<FeatureCard key={idx} {...mod} />))}</div></div>
    </section>
  );
};
...
const StartupBundle = () => {
  return (
    <section className="py-20 px-6"><div className="max-w-7xl mx-auto"><div className="gradient-border"><div className="gradient-border-inner bg-gradient-to-br from-gray-900 to-gray-950"><div className="grid md:grid-cols-2 gap-8 items-center">
...
const CliPreview = () => {
  ...
  return (
    <section className="py-20 px-6 bg-gray-900/30" id="cli"><div className="max-w-7xl mx-auto text-center"><h2 className="text-3xl font-bold mb-3">Deploy from your terminal</h2><p className="text-gray-400 max-w-2xl mx-auto mb-10">The PCLOUDs CLI gives you full control. Push updates, manage env vars, and stream logs.</p><div className="max-w-2xl mx-auto code-block text-left"><AnimatedCode lines={commands} /><div className="mt-4 pt-3 border-t border-gray-800 flex justify-between items-center"><span className="text-xs text-gray-500">pclouds-cli v1.0.0-beta</span><span className="text-green-400 text-sm flex items-center gap-1"><span className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></span> connected</span></div></div></div></section>
  );
};
...
const CodeScreenshots = () => {
  return (
    <section className="py-20 px-6 bg-gray-900/20"><div className="max-w-7xl mx-auto"><div className="text-center mb-12"><h2 className="text-3xl font-bold mb-3">Build with your favorite tools</h2><p className="text-gray-400">Seamless integration into your existing workflow</p></div><div className="grid md:grid-cols-2 gap-8"><CodeEditorScreenshot title="deploy-app.tsx" language="TypeScript" /><CodeEditorScreenshot title="api-usage.js" language="JavaScript" /></div><div className="mt-8 max-w-2xl mx-auto"><CodeEditorScreenshot title="Dockerfile" language="dockerfile" /></div></div></section>
  );
};
...
// ---------- MAIN APP (rocket + globe directly after hero) ----------
function App() {
  return (
